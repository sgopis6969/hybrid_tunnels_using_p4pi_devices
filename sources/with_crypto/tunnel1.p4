...
/***anything other than 20 bytes 5 x 32 bits = 20 bytes ***/
		...
		meta.label_id = hd.label.label_id;
		transition accept;
		}
                  ...
                  inout standard_metadata_t standard_metadata)
{
		...
		action transport_decap(bit<9> port) {
		 ...
		 action transport_out(bit <9> port) {
    ...
    apply {
		...
		if( standard_metadata.ingress_port == 2) {
			...
			if(meta.is_tunneled == true){
				...
				transport_out_table.apply(); 
				}

			/*standard_metadata.egress_spec = 1;*/
			/*standard_metadata.egress_spec = meta.outport;*/
			/*log_msg("Debug: Port2");*/
			}
